                                    ...
                                    <ul class="hor-ul">
                                        <li>
											<button name="add_itm_upldimg" class="btn btn-primary add_background_image" data-toggle="tooltip" title="Set New Background" type="">
                                                <i class="fa fa-camera"></i>
												<input type="file" class="backhround_image" name="upload_image" id="">
                                            </button>
                                        </li>
                                        <li>
                                            <button class="btn btn-primary crop_background_image" data-toggle="tooltip" title="Crop Image" type="button">
                                                <i class="fa fa-crop"></i>
                                            </button>
                                        </li>
                                        <li>
                                            <button class="btn btn-primary" data-toggle="tooltip" title="Visibility of labels" type="button" style="margin-right: 0.7%;">
...
$(document).ready(function() {
	...
	$('.bdrrcolor_picker').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			$( '.print-list-datamodal .modal-body .contain-div' ).css( {'color': '#' + hex} );                    
		}
	});
	$(".print-prev-bordersi:input[type=number]").bind('keyup mouseup', function () {
		$( '.print-list-datamodal .modal-body .contain-div' ).css( {'border': ''} );
		$( '.print-list-datamodal .modal-body .contain-div' ).css( {'border': $( this ).val() + 'px solid'} );
	});	
	...
	$('.save-style').click(function(){
		...
		var arraylabels = labels_value.split(";");
		var labelsArray = [];
		
		labelsArray.push( { Key : 'text' , Value : labels_text } );
		...
		if( labels_text == '' ){
			...
			$.ajax({
                ...
                type: "POST",            
                data:$(this).serialize()+'&tblprint-id='+tblprint_id+'&labels-value='+labelsArray ,                
				success: function(data)   
				{
					console.log("DATA=> "+data);
					//console.log("DATA=> "+data+ 'id=>'+ tblprint_id);
				}
            });
			return false;
		}
...
$(document).ready(function(){
	...
	/*Popup Image upload */
	$( '.backhround_image' ).on( 'change', function(){
		var reader = new FileReader();
		reader.onload = function ( event ) {
			$image_crop.croppie( 'bind', {
	...
	$('.crop_background_image').click(function(event){
		//alert('Hello');
	});
